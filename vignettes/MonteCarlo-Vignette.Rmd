...
Different from functions and packages, *MonteCarlo()* currently does not support the automatic export of datasets used in *func* to the cluster. If this is desired, the respectve dataset has to be supplied as *export_also$data*.
...
A simple way to make these differences visible without displaying to many digits is to change the unit, for exampe by multiplying all results with 100. This is where the *transform* argument of *MakeTable()* comes in handy.
