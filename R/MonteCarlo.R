...
MC.inner<-function(func,M,param.list, ret.vals, ncpus=1, raw=FALSE, max.grid=1000, packages=NULL, export.functions=NULL, DEBUG=FALSE){
  ...
  eval(parse(text=paste("func2<-function(","aux,",subm.param,"){func(",subm.param,")}")))  
  ...
  in.export<-paste("'",c('func2','func',export.functions,param.names),"'", collapse=",", sep="")
  ...
  if(DEBUG==TRUE){
    iterator<-c("M","ncpus","rep","pb","func","func2","results")
    for(i in 1:length(iterator)){assign(iterator[i],get(iterator[i]),envir = .GlobalEnv)}
    for(i in 1:n.param){assign(paste(param.names[i],".grid",sep=""),param.list[[i]], envir = .GlobalEnv)}
...
#' @details All input arguments have to be scalar, function has to return list of named scalars, list elements have to be unnamed scalars.
...
MonteCarlo<-function(func, M, param.list, ncpus=1, raw=TRUE, max.grid=1000, timeNtest=FALSE, save.res=TRUE, DEBUG=FALSE){
  ...
  for(i in 1:length(param.list)){if(is.vector(param.list[[i]])==FALSE)stop("Parameter grids have to be vectors.")}
  ...
  if(is.list(test.run)==FALSE)stop("func has to return a list with named components. each component has to be scalar.")
  for(i in 1:length(test.run)){if(is.null(names(test.run[[i]]))==FALSE)stop("Scalars in list components cannot be named.")}
  for(i in 1:length(test.run)){if(length(test.run[[i]])>1)stop("func has to return a list with named components. each component has to be scalar.")}
  ...
  for(i in 1:length(all)){
    if(is.function(eval(parse(text=all[i])))){all.funcs<-c(all.funcs,all[i])}
  }
  ...
  in.func.aux2<-gsub("<-","",in.func.aux)
  in.func.aux<-backtick_binaries(in.func.aux2[in.func.aux2!=""])
  ...
  while(length(new.funcs)>0){ # while loop runs as long as new functions are found in deeper layers 
    ...
    for(i in 1:n.exp){# find functions used in every new function
      ...
      if(length(in.inner)>0){all.funcs.found<-unique(c(all.funcs.found,in.inner[apply(as.matrix(in.inner),1,exists)]))} # determine which of the functions defined in inner functions do exist and append to list of all functions that may come from packages
    }
  ...
  sort.out<-which(packages%in%dependencies.list)
  if(length(sort.out)>0){packages<-packages[-sort.out]}  # keep only those packages that are not automatically included because they are dependencies
  }
  ...
  if(timeNtest==TRUE){
    ...
    t1<-Sys.time()
    erg.pre<-MC.inner(func=func,M=M/10,param.list=param.list2, ret.vals=ret.vals, ncpus=ncpus, raw=raw, max.grid=max.grid, packages=packages, export.functions=export.functions, DEBUG=DEBUG)
    t2<-Sys.time()
  ...
  erg<-MC.inner(func=func,M=M,param.list=param.list, ret.vals=ret.vals, ncpus=ncpus, raw=raw, max.grid=max.grid, packages=packages, export.functions=export.functions, DEBUG=DEBUG)
  return(erg)  
}
