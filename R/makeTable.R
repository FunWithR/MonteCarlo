...
#' for each value of the grid for the fourth parameter can again be stacked into rows or columns and so on. Confer the example.
#' 
...
#' 
#' If it is desired to use include the list elements in a single table, this behavior can be modified by adding "list" 
...
#' @param include_meta booloean that determines whether the meta data provided by summary() is included in comments below the table.
#' Default is \code{include_meta==TRUE}.
#' @param partial_grid an optional list with the elements named after of the parameters for which only a part of the grid values is
...
#'  
#'  # return results
#'  return(list("mean for calculation of sd"=mean_sample, "bias_mean"=bias_mean_sample, "median for calculation of sd"=median_sample,"bias_median"=bias_median_sample))
#' }
#'
#' n_grid<-c(50,100,250,500)
#' loc_grid<-seq(0,1,0.2)
...
#' MakeTable(output=erg_mean_median, rows=rows, cols=cols, digits=2, collapse=collapse, include_meta=FALSE)
#'
#' # merge all results in one table
#'
#' MakeTable(output=erg_mean_median, rows=c("n","loc"), cols=c("scale","list"), digits=2, collapse=collapse, include_meta=FALSE)
#'
#' # transform the results for better scaling
#' 
#' scale_table_10<-function(x){x*10}
#'
#' MakeTable(output=erg_mean_median, rows=c("n","loc"), cols=c("scale","list"), digits=2, collapse=collapse,
