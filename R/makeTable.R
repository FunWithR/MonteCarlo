...
#' @keywords internal
collapse.results<-function(output){
ret<-list()
...
loop<-paste(paste(paste("for(i",1:num.param," in 1:",dim(out)[1:num.param],"){", sep=""), collapse=""),paste(c("erg[",help,"]<-"), collapse=""),paste(c("mean(out[",help,",])"),collapse=""),paste(rep("}",num.param),collapse=""),collapse="")
...
#' @details add details
#' parameter grids of length one are dropped from tabel and values are added to caption
...
MakeTable<-function(output, rows, cols, digits=4, Transform=NULL, func=NULL){
  ...
  output<-collapse.results(output) # from raw output average over results, so that monte carlo results are saved in list of large arrays
  ...
  out<-round(output[[lll]], digits=digits)      # select element from list and round result to specified number of digits.
  ...
  if(any(dim(out)==1)){
    ...
    eval(parse(text=paste(c(sel.str,"]"), collapse="")))
  ...
  build.string<-paste(c(
    if(loops.cols>0){paste("for(j",1:loops.cols," in 1:","col.dims[",1:loops.cols,"]){", sep="")}else{""},
    "inner.rows<-NULL;",
    if(loops.rows>0){paste("for(i",loops.rows:1," in 1:","row.dims[",1:loops.rows,"]){", sep="")}else{""},
    paste("inner.rows<-rbind(inner.rows,out[,,",ind,"],rep(NA,dims[2]))"),
    rep("}",loops.rows),
    ";",
  ...
  }else{head1<-c(paste(get(paste(cols[1],".grid",sep=""))),NA)}
  heads[[1]]<-head1
  # inner rownames
  
  if(length(rows)>1){help.rows1<-eval(parse(text=paste("rep(c(paste(",paste(rows[1],".grid",sep=""),"),NA),row.dims[1])",sep="")))}else{help.rows1<-eval(parse(text=paste("rep(c(paste(",paste(rows[1],".grid",sep=""),"),NA),1)",sep="")))}
